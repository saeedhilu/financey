...
class EMIPlan(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name='emi_plans')
    ...
    liability_ledger = models.ForeignKey(Ledger, on_delete=models.PROTECT, related_name='emi_plans')
    deduct_from_ledger = models.ForeignKey(Ledger, on_delete=models.PROTECT, related_name='emi_deductions')
    start_date = models.DateField()
    is_active = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"{self.name} - {self.months_paid}/{self.total_months}"
